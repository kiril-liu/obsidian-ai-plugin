...
import {
  BatchPromptRunOptions,
  BatchPromptRunResult,
  PromptWorkflowOutputTarget,
} from "../types";
...
export class BatchPromptRunner {
  ...
  async run(options: BatchPromptRunOptions): Promise<BatchPromptRunResult> {
    const template = await this.plugin.promptManager.getTemplateByName(
      options.templateName,
    );

    if (!template)
      throw new Error(`没有找到 Prompt 模板：${options.templateName}`);
    ...
    this.plugin.progressTracker.start(`批量运行：${template.name}`, [
      "准备文件",
      "逐个调用 AI",
      "写入结果",
      "完成",
    ...
    for (const filePath of filePaths) {
      ...
      try {
        ...
        const answer = await this.plugin.aiClient.chat(
          prompt,
          this.plugin.progressTracker.getSignal(),
        );
    ...
    this.plugin.progressTracker.complete(
      `批量 Prompt 完成：成功 ${success}，失败 ${failed}`,
    );
  ...
  async writeOutput(
    ...
    const folder = normalizePath(
      outputFolder || this.plugin.settings.workflowOutputFolder,
    );
    ...
    const outputPath = normalizePath(
      `${folder}/${safeInputName} - ${safeTemplateName}.md`,
    );
    ...
    else await this.plugin.app.vault.create(outputPath, body);
