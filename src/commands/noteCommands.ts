...
export function registerNoteCommands(plugin: AiPlugin) {
	plugin.addCommand({
		...
		editorCallback: async (editor) => {
			await runEditorCommand(plugin, editor, "请总结下面这段内容：", "replace_selection");
		},
	});
	...
	plugin.addCommand({
		...
		editorCallback: async (editor) => {
			await runEditorCommand(plugin, editor, "请润色改写下面这段内容，保持原意：", "replace_selection");
		},
	});
	...
	plugin.addCommand({
		...
		callback: async () => {
			...
			await runPromptForNote(plugin, `请总结下面这篇笔记：\n\n${plugin.limitText(content)}`, "append_to_note");
		},
	});
	...
	plugin.addCommand({
		...
		callback: async () => {
			...
			const content = await plugin.app.vault.cachedRead(view.file);
			await runPromptForNote(plugin, `请从下面笔记中提取 TODO，使用 Markdown checklist：\n\n${plugin.limitText(content)}`, "append_to_note");
		},
	});
}

async function runEditorCommand(plugin: AiPlugin, editor: Editor, instruction: string, defaultAction: "replace_selection" | "append_to_note") {
	...
	await runWithPreview(plugin, prompt, defaultAction, async (content, action) => {
		applyEditorOutput(editor, action, content);
	});
}

async function runPromptForNote(plugin: AiPlugin, prompt: string, defaultAction: "append_to_note") {
	const view = plugin.app.workspace.getActiveViewOfType(MarkdownView);
	if (!view) return;
...
async function runWithPreview(
	...
	apply: (content: string, action: "insert_at_cursor" | "append_to_note" | "replace_selection") => Promise<void> | void
) {
	try {
		const signal = plugin.progressTracker.start("AI 处理笔记", ["发送请求", "等待回复", "准备输出"]);
