...
export class PromptManager {
  ...
  async loadTemplates(force = false): Promise<PromptTemplate[]> {
    ...
    if (this.plugin.settings.enablePromptLibrary) {
      const file = this.plugin.app.vault.getAbstractFileByPath(
        this.plugin.settings.promptLibraryPath,
      );
    ...
    this.templates = templates.sort((a, b) => {
      const favoriteDiff =
        Number(Boolean(b.metadata.favorite)) -
        Number(Boolean(a.metadata.favorite));
      if (favoriteDiff !== 0) return favoriteDiff;
      return `${a.metadata.category ?? ""}${a.name}`.localeCompare(
        `${b.metadata.category ?? ""}${b.name}`,
      );
    });
  ...
  parseYamlMetadata(body: string): PromptTemplateMetadata {
    ...
    for (const line of match[1].split("\n")) {
      ...
      if (key === "mode" && ["basic", "keyword", "rag"].includes(value))
        metadata.mode = value as any;
      if (
        key === "output" &&
        ["chat", "append", "replace_selection"].includes(value)
      )
        metadata.output = value as any;
  ...
  buildRenderContext(
    partial: Partial<PromptRenderContext> = {},
  ): PromptRenderContext {
    const now = new Date();
    ...
    return {
      ...
      chatHistory: this.plugin.chatHistory
  ...
  async createLifePromptPack() {
    ...
    if (existing instanceof TFile) {
      await this.plugin.app.vault.modify(existing, LIFE_PROMPT_PACK);
    } else {
      await this.plugin.app.vault.create(path, LIFE_PROMPT_PACK);
    }
